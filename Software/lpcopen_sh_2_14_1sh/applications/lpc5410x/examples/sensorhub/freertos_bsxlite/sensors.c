...
extern QueueHandle_t qSenEvent;
...
{
	uint32_t val;
	BaseType_t hWake, ret;
	PhysicalSensor_t *pSens;
	configASSERT(sensorId < PHYS_MAX_ID);
	pSens = g_phySensors[sensorId];
	if (!pSens)
		return ;
	
	configASSERT(pSens->id == sensorId);
	if (enable == pSens->enabled)
		return ;
	val = (uint32_t) sensorId | SENSOR_EVENT_ENABLE | (enable << 17);
	
	/* FIXME: Once this code becomes a task use xQueueSend */
	ret = xQueueSendFromISR(qSenEvent, &val, &hWake);
	portEND_SWITCHING_ISR(ret && hWake);
}
...
{
	BaseType_t ret, hWake = pdFALSE;
	uint32_t senid = PHYS_ACCEL_ID | SENSOR_EVENT_READ;
	Chip_PININT_ClearIntStatus(LPC_PININT, ACCEL_PINT_CH);

	g_phySensors[PHYS_ACCEL_ID]->ts_lastSample = xTaskGetTickCountFromISR();
	ret = xQueueSendFromISR(qSenEvent, &senid, &hWake);
	portEND_SWITCHING_ISR(ret && hWake);
}
...
{
	BaseType_t ret, hWake = pdFALSE;
	uint32_t senid = PHYS_GYRO_ID | SENSOR_EVENT_READ;
	Chip_PININT_ClearIntStatus(LPC_PININT, GYRO_PINT_CH);

	g_phySensors[PHYS_GYRO_ID]->ts_lastSample = xTaskGetTickCountFromISR();
	ret = xQueueSendFromISR(qSenEvent, &senid, &hWake);
	portEND_SWITCHING_ISR(ret && hWake);
}
...
{
	BaseType_t ret, hWake = pdFALSE;
	uint32_t senid = PHYS_MAG_ID | SENSOR_EVENT_READ;
	Chip_PININT_ClearIntStatus(LPC_PININT, MAG_PINT_CH);
	g_phySensors[PHYS_MAG_ID]->ts_lastSample = xTaskGetTickCountFromISR();
	ret = xQueueSendFromISR(qSenEvent, &senid, &hWake);
	portEND_SWITCHING_ISR(ret && hWake);
}
...
{
	BaseType_t ret, hWake = pdFALSE;
	uint32_t senid = PHYS_PROX_ID | SENSOR_EVENT_READ;
	Chip_PININT_ClearIntStatus(LPC_PININT, PROXI_PINT_CH);

	g_phySensors[PHYS_PROX_ID]->ts_lastSample = xTaskGetTickCountFromISR();
	ret = xQueueSendFromISR(qSenEvent, &senid, &hWake);
	portEND_SWITCHING_ISR(ret && hWake);

#if(SENSOR_AMBI==AMBI_AMSCT1010)
	senid = PHYS_AMBIENT_ID | SENSOR_EVENT_READ;
	hWake = pdFALSE;

	g_phySensors[PHYS_PROX_ID]->ts_lastSample = g_phySensors[PHYS_PROX_ID]->ts_lastSample;
	ret = xQueueSendFromISR(qSenEvent, &senid, &hWake);
	portEND_SWITCHING_ISR(ret && hWake);
#endif
}
